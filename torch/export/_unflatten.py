...
class UnflattenedModule(torch.nn.Module):
    def __init__(
        ...
        # Fixes all the buffer mutations -- DUPE FOR UNLIFT
        _inplace_buffer_mutations(export_graph, self.graph_signature)

        # Moves nodes to their submodules
        ...
        # Puts params/buffers into their submodules, creates empty modules for
        # those that dont exist -- DUPE FOR UNLIFT
        ...
        # Unlifts params/buffers/constants to getattr nodes -- DUPE FOR UNLIFT
        # Also unlifts from call_module nodes and into submodules
    ...
    def forward(self, *args, **kwargs):
        # Why is there this custom forward function
        ...
        if is_fx_tracing():
            # So this is still technically a GraphModule bc we use Interpreter.
            # It's still not clear to me why this can't be GraphModule?
            # why is this fx_tracing done here?
...
def _is_prefix(candidate, target):
    ...
    meaning that the target path is a sub-path of the candidate path?
    """
...
class _ModuleFrame:
    def __init__(
        ...
        if signature is not None and self.parent is not None:
            # We only restore signature for those with module_call_graph annotated?
            breakpoint()
