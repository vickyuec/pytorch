...
Tensor _s_poisson_cuda(const Tensor& lambda, const c10::optional<Generator>& gen_) {
...
Tensor _s_binomial_cuda(const Tensor& count, const Tensor& prob, const c10::optional<Generator>& gen_) {
  auto gen = get_generator_or_default<CUDAGeneratorImpl>(gen_, cuda::detail::getDefaultCUDAGenerator());
...
Tensor _s_gamma_cuda(const Tensor& alpha, const c10::optional<Generator>& gen_) {
  auto gen = get_generator_or_default<CUDAGeneratorImpl>(gen_, cuda::detail::getDefaultCUDAGenerator());
...
Tensor _s_dirichlet_cuda(const Tensor& alpha, const c10::optional<Generator>& gen_) {
  auto gen = get_generator_or_default<CUDAGeneratorImpl>(gen_, cuda::detail::getDefaultCUDAGenerator());
  Tensor ret = at::empty(alpha.sizes(), alpha.options());
  launch_gamma_kernel(ret, alpha, gen);
